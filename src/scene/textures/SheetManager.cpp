...
// Single Sprite
void SheetManager::create_sheet(int sheet_id, SDL_Texture* texture, SDL_Rect r, uint8_t renderType) {
	if (spriteSheets.find(sheet_id) == spriteSheets.end()) {
		spriteSheets[sheet_id] = SpriteSheet(texture, renderType);
		spriteSheets[sheet_id].add_frame(IDLE, {r, -1, FRAME_STOP});
	}
	else
...
// Single Animation
void SheetManager::create_sheet(int sheet_id, SDL_Texture *texture, std::vector<Frame> frames, uint8_t renderType) {
	if (spriteSheets.find(sheet_id) == spriteSheets.end()) {
		spriteSheets[sheet_id] = SpriteSheet(texture, renderType);
		spriteSheets[sheet_id].add_frames(IDLE, std::move(frames));
	}
	else
		printf("Err: Attempted to create sheet with pre-existing ID.\n");
}
...
// Full Sprite
void SheetManager::create_sheet(int sheet_id, SDL_Texture *texture, std::vector<std::pair<int, std::vector<Frame>>> frameData, uint8_t renderType) {
	if (spriteSheets.find(sheet_id) == spriteSheets.end()) {
		spriteSheets[sheet_id] = SpriteSheet(texture, renderType);
...
SDL_Texture * SheetManager::get_sheet_shadow(
	const Object *o) {
	return spriteSheets[o->sheet_id()].get_shadow();
}

void SheetManager::set_sheet_shadow(int sheet_id,
                                    SDL_Texture *texture) {
...
SDL_Texture * SheetManager::get_sheet_texture(
	const Object *o) {
...
SDL_Rect * SheetManager::get_sheet_frame(Object *o) {
...
string SheetManager::to_string() {
	string ret = "SheetManager::to_string():\n"
