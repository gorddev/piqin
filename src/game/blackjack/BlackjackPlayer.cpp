...
BlackjackPlayer::BlackjackPlayer() {
    // Set up each of our member objects
    slct = new CardSelector();
    deck = new Deck();
    hand = new Hand();
    // Now for our player and opponent
    playerDraw = new CardDraw(BJ_DEFAULT_PLAYER_POS); // NOLINT(*-integer-division)
    opponentDraw = new CardDraw(BJ_DEFAULT_OPPONENT_POS); // NOLINT(*-integer-division)

    // Now we set up our menu region!
    discardTemp = new std::vector<Card *>(1);
    (*discardTemp)[0] = &deck->discard;
    mnu = new MenuRegion<Card>(
        {
            &hand->cards,
    ...
    mnu->set_selectables({BJ_TARGET_HAND});
...
std::vector<Object*> BlackjackPlayer::gather_objects() const {
...
void BlackjackPlayer::play_card_from_hand() {
    ...
    if (slct->get_target()->get_suit() == SPECIAL_CARD_SUIT)
