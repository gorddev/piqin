...
#define ADDEV(t, f) ganine::GENG_Events.add_event(t, [=]{f;})
...
void Round::get_release(ganine::GENG_Input keybind) {
    if (acceptingInput)
...
bool Round::get_press(ganine::GENG_Input keybind) {
    if (acceptingInput) {
        pather.update_hand(board->hand);
        if (keybind == ganine::GENG_Input::BACK && board->floater == nullptr) {
