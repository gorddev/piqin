...
SDL_FRect RenderManager::rect_flat(Object& o) const {
    float x = o.x() - (o.width() / 2.0 + cam->pos.x);
    float y = o.y() + (o.height() / 2.0 + cam->pos.y);
...
SDL_FRect RenderManager::rect_shadow(Object& o) const {
    ...
    float px = static_cast<int>(o.width() / 2.0) + (cam->pos.x + o.x());
    float py = static_cast<int>(o.height() / 2.0) + (cam->pos.y + o.y());
    px += static_cast<int>(dx*0.055);
    py += static_cast<int>(dy*0.055);

    // Then we scale the shadows
    double scale = sqrt(sqrt((dx*dx) + (dy*dy))) * 0.005;
    float w = o.width() * (1 + scale);
    float h = o.height() * (1 + scale);
