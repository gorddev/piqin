...
void Shake::shake_circular(Transform2D& tref, LayerState& time) const {
    float div = time.get_time() * speed* 0.005f* 2*gutils::pi*((!permanent) ? 1 : 1);
    tref.offset.x = cosf(div);
    tref.offset.y = sinf(div);
}

void Shake::shake_floaty(Transform2D& tref, LayerState& time) const {
...
void Shake::shake_side(Transform2D& tref, LayerState& time) const {
