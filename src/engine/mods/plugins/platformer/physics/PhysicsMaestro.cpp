#include "engine/mods/plugins/platformer/physics/PhysicsMaestro.hpp"
#include "engine/mods/plugins/platformer/world/WorldManager.hpp"
#include "engine/mods/plugins/platformer/physics/TileContact.hpp"
...
PhysicsMaestro::PhysicsMaestro(LayerCore& scene, WorldManager &world)
...
inline bool tile_overlap(const fbox2& a, const box2& b) {
    return !(a.x + a.w <= b.x ||
             a.x >= b.x + b.w ||
             a.y + a.h <= b.y ||
             a.y >= b.y + b.h);
...
void PhysicsMaestro::collide(TileGrid& grid, int& id) {
    ...
    vec2 start = prev_positions[id];
    vec2 vel = c.t.pos - vec2{c.get_hitbox().w/2.f - c.get_offset().x, c.get_hitbox().h/2.f - c.get_offset().y} - start; // total intended movement
    dim2 box = c.get_hitbox();
    ...
    vec2 stepDelta = vel / static_cast<float>(steps);
    vec2 pos = start;
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        for (int ty = min_ty; ty <= max_ty; ++ty) {
            for (int tx = min_tx; tx <= max_tx; ++tx) {
                ...
                box2 tile = grid.tile_box(tx, ty);
                fbox2 mover { pos.x, pos.y, box.w, box.h };
    ...
    if (collision) {
        vec2 newPos = pos + vec2{c.get_hitbox().w/2.f - c.get_offset().x, c.get_hitbox().h/2.f - c.get_offset().y};
