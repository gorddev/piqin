...
void EventModule::add_relative_event(double duration, std::function<void()> func) {
    relative_queue.emplace_back(duration, func);
}

void EventModule::add_absolute_event(double duration, std::function<void()> func) {
    absolute_queue.emplace_back(duration, func);
}

void EventModule::add_epoch_event(double duration, std::function<void()> func) {
    epoch_queue.emplace_back(duration, func);
}

void EventModule::clear_epoch() {
    epoch_queue.clear();
}
