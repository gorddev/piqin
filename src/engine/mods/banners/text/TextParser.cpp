...
SDL_Color TextParser::color_from_name(const gan::str_view& name) {
    if (name == "red")    return {max_alpha, 60, 60, max_alpha};
    if (name == "green")  return {100, 200, 100, max_alpha};
    if (name == "blue")   return {140, 140, max_alpha, max_alpha};
    if (name == "orange") return {max_alpha, 165, 0, max_alpha};
    if (name == "white")  return {max_alpha, max_alpha, max_alpha, max_alpha};
    if (name == "black")  return {0, 0, 0, max_alpha};
    return {max_alpha, max_alpha, max_alpha, max_alpha};
}
...
ParseEvent TextParser::parse_next(const gan::str_view& src, int pos) {
    ...
    if (cmd.substr<2>(0) == "c:") {
        ...
        int nums[4] = {max_alpha, max_alpha, max_alpha, max_alpha};
        int count = 0;
        uint16_t p = 0;
