...
SparkleInst::SparkleInst(const vec2 &offset, float speed, float size) {
    pos = offset;
    velocity.randomize();
...
Sparkle::Sparkle(vec2 pos, float size, float speed, float duration, float frequency, SDL_Color Tint)
    : ParticleGroup(pos, size, speed, duration, {gan::max_alpha, gan::max_alpha, gan::max_alpha, gan::max_alpha}), period(frequency) {
    color = Tint;
    if (duration == -1)
        permanent = true;
}
...
Sparkle::Sparkle(Gear* o, float size, float speed, float duration, float period, SDL_Color Tint)
    : ParticleGroup(o, size, speed, duration, {gan::max_alpha, gan::max_alpha, gan::max_alpha, gan::max_alpha}), period(period) {
    color = Tint;
    if (duration == -1)
        permanent = true;
}
    ...
    if (!done) {
        while (deltat > period) {
            ...
            if (payload != nullptr) {
                ...
                particles.emplace_back(payload->t.offset + payload->t.pos - vec2(0,0), speed, strength);
            }
            else
