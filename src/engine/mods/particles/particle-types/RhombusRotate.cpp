...
RhombusRotateInst::RhombusRotateInst(const vec2 &offset, float speed, float size) {
    pos = offset;
    velocity.randomize();
    velocity = velocity.unit();
    velocity *= speed;
    radius = static_cast<short>(size);
}

bool RhombusRotateInst::update(const double& dt) {
    duration -= static_cast<float>(dt);
    if (duration <= 0)
        return true;
    pos.x += static_cast<float>(velocity.x * dt* 0.05);
    pos.y += static_cast<float>(velocity.y * dt* 0.05);
    return false;
}

void RhombusRotateInst::to_vertex(RenderBuffer& buffer, SDL_Color& color) const {
    // Radius
    const float rad = (static_cast<float>(radius) * duration / 1500.f);
    // FPos2D locations

    const float angle = duration / 100.f;
...
RhombusRotate::RhombusRotate(vec2 pos, float size, float speed, float duration, float frequency, SDL_Color Tint)
...
bool RhombusRotate::update(LayerState &time) {
    ...
    deltat += static_cast<float>(time.get_dt());
