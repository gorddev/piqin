...
void Font::add_char(char c, dim2 atlas_dim, pos2 tex_pos, dim2 tex_dim, uint16_t advance) {
    if (has_character(c)) {
        std::vector<SDL_FPoint> vertexPoints(4);
        float u0 = (tex_pos.x - 0.0f) / atlas_dim.w;
        float v0 = (tex_pos.y - 0.0f) / atlas_dim.h;
        float u1 = (tex_pos.x + tex_dim.w+0.f) / atlas_dim.w;
        float v1 = (tex_pos.y + tex_dim.h+0.f) / atlas_dim.h;
