...
struct Transform {
    ...
    Vertex vel = Vertex(0,0,0);

    Transform() = default;
    Transform(Vertex pos) : pos(pos) {}
    Transform(Vertex pos, Vertex velocity) : pos(pos), vel(velocity) {}
    ...
    Transform(Vertex pos, Vertex velocity, double height, double width, float scale, float z_index) : pos(pos), vel(velocity), height(height), width(width), scale(scale), z_index(z_index) {}
};