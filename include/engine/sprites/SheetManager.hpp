...
#include "asset-info/HeaderRegistry.hpp"
#include "asset-info/SpriteRegistry.hpp"
// Include Object?
...
namespace gengine {
	...
	public:
		...
		void create_sheet(int sheet_id, SDL_Texture* texture, SDL_Rect r);
		// Create a sheet with an animation of repeating frames.
		void create_sheet(int sheet_id, SDL_Texture* texture, std::vector<Frame>);
		// Creates a sheet with a fully fledged spriteset
		void create_sheet(int sheet_id, SDL_Texture* texture, std::vector<std::pair<int, std::vector<Frame>>> frameData);
		...
		SDL_Rect* get_sheet_frame(const Object& o);
