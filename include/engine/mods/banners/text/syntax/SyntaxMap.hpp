...
namespace gan {
    ...
    private:
        ...
        std::vector<SyntaxCommand> commands;
    ...
    public:
        ...
        SyntaxMap(std::vector<SyntaxCommand> commands,
            std::vector<SyntaxPattern> patterns,
            std::vector<SyntaxHighlight> highlights)
                : commands(std::move(commands)), patterns(std::move(patterns)),
        ...
        hstring parse(str_view text, Font& font, std::vector<FontChar>& buffer, std::vector<SyntaxInfo>& syntax_info) const {
            ...
            // Length of the text
            long len = std::min(text.length(), text.length());
