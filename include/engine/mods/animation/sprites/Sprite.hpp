...
namespace gan {
    /**
     * @brief A @code ganine::Sprite@endcode is a simple game entity that carries with it a @code ganine::Transform2D@endcode
     * and a @code ganine::FrameState@endcode.
     * It is one of the primary three objects the engine performs operations on (that being ParticleGroups, Sprites, and Panels).
     * @details The Sprite is a subclass of the @code gan::Gear@endcode, which means that it inherits many of their nice features.
     ...
     * - @code virtual void move(gan::FPos2D dist)@endcode › Called when the cursor drags the object any distance @code dist@endcode.
    ...
    public:
        ...
        explicit Sprite(FrameTable& frames);
